...
Choose `picked.fasta`, `picked.count_table` and `sequences-taxonomy-assignment.txt`. 
Next, tun the tool `Generate input files for phyloseq` so that you select the correct data type (`16S, 18S or archaeal`) and set a dissimilarity threshold of 0.03% for OTU clustering.

```
Why are we using a threshold of 0.03%? 
...
Fill the `chiptype` column so that samples `F3D0-F3D147` (moving downward from the top row) are marked as `a`, and that samples from `F3D148` onward are marked as `b`. You should end up with 9 samples with the chiptype marked as `a` and 10 samples with `b`.

ii) Next, fill in the `group` column: mark all the early samples (`D0-D9`) with `a`, and the late samples (`D141-D150`) with `b`.
...
**Step 18. Stacked taxon composition bar plots**
...
```
**Step 19. Other ways to visualize the data**
