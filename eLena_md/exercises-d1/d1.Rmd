...
Based on the plot Sequence Quality Histograms, is the base quality good all along the reads? 
Do the samples marked with red have something in common (click the traffic light bar above the plot)?
```
...
How many sequences are there in the data? How long are most of the contigs? The longest contig? Are there ambiguous bases in the contigs?

Select contig.numbers.txt and view it as Text. Are there roughly the same number of contigs in each sample?

Check in the samples.fastqs.txt if the fastq files were assigned correctly to samples.
Open the contigs.groups files as a spreadsheet. What information does it contain?
```
...
```
Compare the number of chimeras removed by these two methods: Check the two files called chimeras.removed.summary.tsv. 
